...
<Callout type="info">
    **Recommendation**: Use `/dev/shm` unless you have RAM constraints or need
...
<Callout type="warn">
    **Don't over-allocate**: Leaving RAM for other services is important. VFS
    will function with a small cache, just with more network requests.
</Callout>
...
<Callout type="info">
    You can use **both**: Set `cache_eviction=TTL` with a generous
    `cache_ttl_seconds`. Old data gets removed by TTL, and if cache is still
    full, LRU kicks in.
</Callout>
...
<Callout type="info">
    **Rule of thumb**: Prefetch should cover ~10-30 seconds of playback. For a
    50 Mbps 4K stream, 30 seconds = ~187 MB. Set `fetch_ahead_chunks`
    accordingly.
</Callout>
